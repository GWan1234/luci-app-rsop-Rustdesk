...
return view.extend({
	...
	render: function() {
		...
		this.keyNote = E('div', { 'class': 'alert-message warning' },
		...
		var view = E('div', { 'class': 'cbi-map' }, [
			...
			E('fieldset', { 'class': 'cbi-section' }, [
				...
				E('div', { 'class': 'cbi-section-descr' }, [ _('Overall'), ': ', this.badge ]),
				table,
				E('div', { 'class': 'cbi-section-actions' }, [ this.startBtn, ' ', this.stopBtn ])
			]),

			E('fieldset', { 'class': 'cbi-section' }, [
				E('legend', {}, _('Server Key')),
				E('div', { 'class': 'cbi-section-descr' },
					_('The server key is generated on the first start and stored at /etc/rustdesk/id_ed25519.pub.')),
				this.keyInput,
			...
			E('fieldset', { 'class': 'cbi-section' }, [
				...
				E('div', { 'class': 'alert-message warning' },
					_('If you cannot connect to your RustDesk Server from the public internet, please make sure that TCP and UDP ports 21114-21119 are opened in the firewall and forwarded to this device.'))
			])
		]);
	...
	refreshStatus: function() {
		...
		return callServiceList('rsop').then(function(data) {
			...
			return callRcList().then(function(data) {
				var svc = (data && data.rsop) ? data.rsop : null;
			...
			self.keyNote.textContent = _('The server key is generated on the first start and stored at /etc/rustdesk/id_ed25519.pub.');
		}).catch(function() {
			self.keyInput.value = '';
			self.keyNote.className = 'alert-message warning';
			self.keyNote.textContent = _('No key yet. Start the service once to generate it.');
		});
	},
	...
	handleAction: function(start, ev) {
		...
		if (!start && !confirm(_('Are you sure you want to stop the RustDesk Server?')))
			return;

		this.startBtn.disabled = true;
		this.stopBtn.disabled = true;

		callRcInit('rsop', start ? 'start' : 'stop').then(function(ret) {
			var ok = !ret;

			ui.addNotification(null, E('p', {}, ok
				? (start ? _('Service started.') : _('Service stopped.'))
				: (start ? _('Failed to start the service.') : _('Failed to stop the service.'))));

			self.refreshStatus();
		}).catch(function(e) {
			ui.addNotification(null, E('p', {},
				(start ? _('Failed to start the service.') : _('Failed to stop the service.')) + ': ' + (e.message || '')));
	...
	handleCopyKey: function(ev) {
		...
		var done = function(ok) {
			ui.addNotification(null, E('p', {}, ok ? _('Copied!') : _('Copy failed')));
		};
